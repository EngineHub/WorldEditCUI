...
public final class LegacyVanillaPipelineProvider implements PipelineProvider {
    ...
    public static class DefaultTypeFactory implements BufferBuilderRenderSink.TypeFactory {
        ...
        private static final RenderSetup QUADS_SETUP = RenderSetup.builder(
            RenderPipeline.builder(RenderPipeline.builder(RenderPipelines.MATRICES_PROJECTION_SNIPPET)
                    ...
                    .buildSnippet())
                .withLocation("pipeline/wecui_legacy_quads")
                .withCull(false)
                .build()
        ).bufferSize(1536).sortOnUpload().createRenderSetup();
        private static final BufferBuilderRenderSink.RenderType QUADS = new BufferBuilderRenderSink.RenderType(
            VertexFormat.Mode.QUADS,
            DefaultVertexFormat.POSITION_COLOR,
            RenderType.create("wecui_legacy_quads", QUADS_SETUP)
        );
        private static final BufferBuilderRenderSink.RenderType LINES = new BufferBuilderRenderSink.RenderType(
            VertexFormat.Mode.LINES,
            DefaultVertexFormat.POSITION_COLOR_NORMAL_LINE_WIDTH,
            RenderTypes.LINES
        );
        private static final BufferBuilderRenderSink.RenderType LINES_LOOP = new BufferBuilderRenderSink.RenderType(
            VertexFormat.Mode.LINES,
            DefaultVertexFormat.POSITION_COLOR_NORMAL_LINE_WIDTH,
            RenderTypes.LINES
        );
